...
async function seed() {
  ...
  const admin = await prisma.user.upsert({
    where: { email: 'admin@resco.edu.ng' },
    update: {},
    create: { email: 'admin@resco.edu.ng', password: adminPw, role: 'ADMIN' },
  });
  console.log('Admin created: admin@resco.edu.ng / admin123');
  ...
  await prisma.teacher.upsert({
    ...
    create: { id: t1User.id, firstName: 'Adebayo', lastName: 'Adeyemi', status: 'ACTIVE', subjects: JSON.stringify(['Mathematics', 'Basic Science']) },
  });
  ...
  await prisma.teacher.upsert({
    where: { id: t2User.id },
    update: { status: 'ACTIVE' },
    create: { id: t2User.id, firstName: 'Funke', lastName: 'Ogunleye', status: 'ACTIVE', subjects: JSON.stringify(['English Language', 'Social Studies']) },
  });

  const t3User = await prisma.user.upsert({
    where: { email: 'bello@resco.edu.ng' },
    update: {},
    create: { email: 'bello@resco.edu.ng', password: teacherPw, role: 'TEACHER' },
  });
  await prisma.teacher.upsert({
    where: { id: t3User.id },
    update: {},
    create: { id: t3User.id, firstName: 'Ibrahim', lastName: 'Bello', status: 'PENDING', subjects: JSON.stringify(['Basic Science', 'CRS']) },
  });
  console.log('Teachers created (2 active, 1 pending)');
  ...
  // ── Teacher Assignments ──
  const a1 = await prisma.teacherAssignment.upsert({
    where: { teacherId_subject_className: { teacherId: t1User.id, subject: 'Mathematics', className: 'JSS1' } },
    update: {},
    create: { teacherId: t1User.id, subject: 'Mathematics', className: 'JSS1' },
  });
  const a2 = await prisma.teacherAssignment.upsert({
    where: { teacherId_subject_className: { teacherId: t1User.id, subject: 'Mathematics', className: 'JSS2' } },
    update: {},
    create: { teacherId: t1User.id, subject: 'Mathematics', className: 'JSS2' },
  });
  const a3 = await prisma.teacherAssignment.upsert({
    where: { teacherId_subject_className: { teacherId: t2User.id, subject: 'English Language', className: 'JSS1' } },
    update: {},
    create: { teacherId: t2User.id, subject: 'English Language', className: 'JSS1' },
  });
  const a4 = await prisma.teacherAssignment.upsert({
    where: { teacherId_subject_className: { teacherId: t2User.id, subject: 'English Language', className: 'SSS1' } },
    update: {},
    create: { teacherId: t2User.id, subject: 'English Language', className: 'SSS1' },
  });
  ...
  const endStr = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000).toISOString();
  ...
  console.log('2 Exams created (both PUBLISHED)');
  ...
  for (const q of engQuestions) {
    await prisma.examQuestion.upsert({
      where: { id: q.id },
      update: {},
      create: { ...q, examId: engExam.id },
    });
  }
  console.log('20 Questions created (10 Math + 10 English)');

  console.log('\n✅ Seed complete!');
  console.log('\n══════════════════════════════════════════════════');
  console.log('  LOGIN CREDENTIALS:');
  console.log('══════════════════════════════════════════════════');
  console.log('  Admin:   admin@resco.edu.ng    / admin123');
  console.log('  Teacher: adeyemi@resco.edu.ng  / teacher123');
  console.log('  Teacher: ogunleye@resco.edu.ng / teacher123');
  console.log('  Teacher: bello@resco.edu.ng    / teacher123 (PENDING)');
  console.log('  Student: john@resco.edu.ng     / student123');
  console.log('  Student: mary@resco.edu.ng     / student123');
  console.log('  Student: peter@resco.edu.ng    / student123');
  console.log('  Student: grace@resco.edu.ng    / student123');
  console.log('  Student: david@resco.edu.ng    / student123');
  console.log('══════════════════════════════════════════════════');
}
